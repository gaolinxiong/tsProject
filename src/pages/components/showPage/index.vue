<template>
    <div class="masonry">
        <div class="everyImg" ref="divs"  v-for="(i, idx) in showTableList">
            <div class="media_download">
                <div @click="handleClickEvent(i.id, 'likes', idx)">
                    <i class="el-icon-circle-check"></i>
                    <span>{{i.likes}}</span>
                    <em>点赞</em>
                </div>
                <div @click="handleClickEvent(i.id, 'favorites', idx)">
                    <i class="el-icon-star-off"></i>
                    <span>{{i.favorites}}</span>
                    <em>收藏</em>
                </div>
                <div @click="handleClickEvent(i.id, 'dislikes', idx)">
                    <i class="el-icon-mouse" ></i>
                    <span>{{i.dislikes}}</span>
                    <em>不喜欢</em>
                </div>
            </div>
            <img ref="imgs" class="item" :url="i.pic_url" :key="i.id"/>
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState('$_main', ['showTableList', 'tableLoading'])
        },
        methods: {
            ...mapMutations('$_main', ['updateImageInfo']),
            ...mapActions('$_main', ['getData', 'updateFieldNum']),
            initPicLength() {
                let divs = this.$refs.divs;
                Array.from(this.$refs.imgs).forEach(function(img,index){
                    ...
                    image.onload = function(){
                        ...
                        let rest = img.getBoundingClientRect();
                        if (rest) {
                            if (rest.bottom < document.documentElement.clientHeight + height) {
                                img.src = src;
                            }
                        }
                        divs[index].style.gridRowEnd = `span ${~~(height/10)}`
                    }
                })
            },
            async init() {
            ...
            async handleFieldNum(id, field) {
                let data = await this.updateFieldNum({
                    id: id,
                    field: field
                });
                console.log(data)
            },
            handleClickEvent(id, field, index) {
                console.log(field, index)
                this.updateImageInfo({
                    index: index,
                    field: field
                });
                this.handleFieldNum(id, field);
            },
            handleScrollEvent(e) {
                console.log(document.body.scrollHeight, document.documentElement.clientHeight, e.target.documentElement.scrollTop)
                let scrollTop = e.target.documentElement.scrollTop || 0;
                let scrollHeight = document.body.scrollHeight || document.documentElement.scrollTop || 0;
